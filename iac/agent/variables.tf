...
variable "iap_iam_enforcement_mode" {
  description = "Agent Gateway IAP authorization mode. \"DRY_RUN\" logs allow/deny decisions without blocking (recommended until validated); null (with fail_open) enforces. Only used when enable_agent_gateway = true."
  type        = string
  default     = "DRY_RUN"
...
variable "authz_fail_open" {
  description = "When true, the gateway ALLOWS a request if an authorization extension is unreachable (safe for rollout). Set false in production to fail closed. Only used when enable_agent_gateway = true."
  type        = bool
  default     = true
}
