...
## VERDICT: **NOT READY TO IMPLEMENT** (apply/enable) yet. **READY to write and review the adoption PR itself**, gated on the tests in Section 4 before enforcement is ever flipped on for real.
...
## 4. Test required before enabling for real — proposed, not run yet

1. **Adopt into Terraform first, zero functional change.** Write the resource matching current live reality exactly (`enable_floor_setting_enforcement = false`, no `sdpSettings` yet), then `terraform import 'google_model_armor_floorsetting.mcp[0]' projects/sreagent-t2-demo/locations/global/floorSetting` (import ID format inferred from the resource's own `name` field — **not yet verified against the provider's import documentation**, flagging this rather than asserting it). `terraform plan` after import should show **0 changes** — that's the proof the code now matches reality. This step alone fixes the drift with no behavior change and is low-risk enough to do without the tests below.
2. **Then, as a separate deliberate change:** flip `enable_floor_setting_enforcement` to `true` (both services stay `inspect_only: true`) and re-run tonight's benign investigation — confirm no regression, same as tonight's real result.
3. **Synthetic detector test (not yet run):** a safe payload constructed to clearly trip one already-configured filter (e.g. a synthetic, obviously-fake secret pattern for `pi_and_jailbreak` or `malicious_uri`, run through the same `list_k8s_events`/`describe_k8s_resource` path) — prove: request/response is inspected, the expected detector returns a real `MATCH` (not `NO_MATCH_FOUND`), `inspect_only` does not block the investigation (`status: done` still), and the `SanitizeOperationLogEntry` log shows the correct verdict. This is the one thing tonight's test deliberately did not do (per your instruction to only use benign traffic).
4. **If SDP is added:** one synthetic SDP payload (a fake-but-recognizable info-type, e.g. a dummy email or fake API-key-shaped string in a K8s label/annotation) through the same path — same 4-part proof.
5. **No genuinely separate non-production GCP project exists for this** — `sreagent-t2-demo` is the only environment used throughout this investigation. Flagging this rather than assuming a "non-prod" project exists; the synthetic tests above are the closest available substitute, run against real infra with synthetic (not real-incident) data.
