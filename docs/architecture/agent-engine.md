...
2. Runs Model Armor input sanitization — **but only if `MODEL_ARMOR_TEMPLATE` is set**, which, per `iac/agent/agent_engine.tf:76-78`, is only set when the Agent Gateway is **disabled**. The live deployment has the gateway **enabled** (`enable_agent_gateway = true`, `iac/agent/terraform.tfvars:9`), so this app-level sanitize step does not currently run. See [Security Operations](../governance/security.md#model-armor) for the full picture — the gateway-side alternative isn't wired either.
