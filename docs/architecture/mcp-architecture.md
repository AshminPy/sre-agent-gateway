...
**RESOLVED 2026-09-07** (issue #86, Section 5 of the new-assignment expansion work): the custom MCP is no longer single-cluster-per-deployment. `get_k8s_clients(cluster_id)` is `@lru_cache(maxsize=32)`, keyed per cluster, resolved through the Terraform-managed registry (`clusters.json`) rather than a single `K8S_MCP_KUBE_CONTEXT` env var. One shared Cloud Run service correctly serves many clusters — including different on-prem clusters via a dynamic Connect Gateway connection built at request time (no static, image-baked kubeconfig file), live-verified end to end (real pod created on the real cluster, retrieved correctly through the deployed service). Adding a new cluster (GKE or on-prem) is now a registry entry + `terraform apply`, never a new deployment or code change. See `docs/connect-gateway-onprem.md`'s "Adding a second on-prem cluster" section and `mcp/tests/test_multi_cluster_isolation.py`'s real concurrent-isolation proof.

## Agent Registry — how MCP servers/tools get registered

**Updated 2026-09-07**: registration is now fully Terraform-managed (issue #33, commit `a3f2ba1`, 2026-09-04) — the imperative `scripts/register_endpoints.py` and `scripts/register_custom_mcp.py` scripts described in earlier versions of this page no longer exist (retired to `archive/RETIRED_2026-09-04_register_endpoints.py`).

Two distinct registration shapes exist in Terraform, for different things:

| Pattern | Used for | Terraform resource | Example |
|---|---|---|---|
| Plain endpoint (no MCP spec) | Google API passthrough hostnames (not MCP tool servers) | `google_agent_registry_service.static` (`iac/agent/agent_registry.tf`) | `*-aiplatform.googleapis.com` |
| `mcp_server_spec { type = "TOOL_SPEC" }` | Real MCP tool servers, including our custom one | `google_agent_registry_service.custom_mcp` (`iac/agent/agent_registry_mcp.tf`) | The tool spec for `sre-k8s-mcp` |

The tool-spec content is `mcp/tool_spec.json` (generated by `scripts/build_mcp_tool_spec.py` from the real `mcp/server.py` tools/list output — Terraform cannot introspect a Python MCP server directly). CI's `terraform-apply.yml` enforces a health-gated ordering so this content never describes tools the currently-serving Cloud Run revision doesn't actually have yet (see `agent_registry_mcp.tf`'s own comment for the exact 3-apply sequence).

The custom MCP is enabled (`enable_custom_mcp = true` in the live deployment) — `google_agent_registry_service.custom_mcp` is a real, active registration, not a no-op, and its `count = var.enable_custom_mcp ? 1 : 0` gating means it simply wouldn't exist as a resource at all if the service were ever disabled again.
