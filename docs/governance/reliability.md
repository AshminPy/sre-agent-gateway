...
| Agent Gateway unavailable | Tool calls fail/hang | No dedicated gateway-health signal today (documented gap) | `authz_fail_open=false` (changed from `true` 2026-09-05, PR #249) — an IAP-specific outage now fails closed, blocking traffic rather than allowing it through; a full gateway outage has no automatic mitigation | See [Gateway Failure runbook](../runbooks/gateway-failure.md) |
